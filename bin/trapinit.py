...
def ask_and_create_directory(dir, message=None):
    #if message:
    ...
    if not os.path.exists(path):
        logging.warning("Directory %s does not exist.", path)
...
def setup_host_config():
    ...
    host_config['default-dirs'] = {'home': homedir}
    if name in ('heastro1', 'heastro2'):
        ...
        host_config['default-dirs'].update({
            'tkp': {'base': tkp_base,
        ...
        host_config['cdesc'] = 'heastro.clusterdesc'
        host_config['sipcfg'] = 'sip.cfg'
        host_config['postgres'] = {'host': '146.50.10.202'}
    return host_config

                    
def create_minimal_tkp_configfile(path, host_config):
...
""" % (host_config['database']['host'],
       host_config['database']['name'],       
       host_config['database']['user'],
       host_config['database']['password']))


def check_tkp_configfile(host_config):
    homedir = host_config['default-dirs']['home']
    path = os.path.join(homedir, '.tkp.cfg')
    try:
        open(path)
    except IOError as exc:
        logging.warning("Can't read the TKP config file at %s", path)
        if ask("Create a minimal TKP config file") == 'y':
            create_minimal_tkp_configfile(path, host_config)
    config = ConfigParser.SafeConfigParser()
    config.read(os.path.expanduser('~/.tkp.cfg'))
    if not config.has_section('database'):
        logging.info("[database] section missing in %s. "
    ...
    for option in ('enabled', 'host', 'name', 'user', 'password'):
        if not config.has_option('database', option):
            logging.info("[database] section is missing option %s in %s. "
                        "Please add and then rerun this script", option, path)
            raise ConfigError("missing database option")
    if not config.getboolean('database', 'enabled'):
        logging.error("Database is not enabled in %s. "
                     "Please enable the database and then rerun this script", path)
        raise ConfigError("database not enabled")
    

def verify_tkp_modules():
    for module in ('tkp', 'tkp.database', 'tkp.database.database',
        ...
        try:
            ...
            logging.error("TKP module %s could not be loaded."
...
def verify_database():
    try:
        ...
        logging.error("MonetDB Python can't be imported: %s\n", str(exc))
        raise
    from tkp.database.database import DataBase
    try:
        database = DataBase()
    except Exception as exc:
        logging.error("Error connecting to the database: %s\n", str(exc))
        raise ConfigError("error connecting to the database: %s\n" % str(exc))
    
    
    
def show_config(host_config):
    dirs = host_config['default-dirs']
    logging.info("The following setup has been detected on the system:")
    logging.info("TKP / TRAP main directory:        %s", dirs['tkp']['base'])
    logging.info("Database (MonetDB) python module: %s", dirs['monetdb']['python'])
    logging.info("Pipeline framework:               %s", dirs['sip']['python'])
    logging.info("LofIm main directory:             %s", dirs['lofim']['base'])
    logging.info("Working (scratch) data directory: %s", dirs['work'])


def check_tkp_installation(host_config):
    try:
        check_tkp_configfile(host_config)
    except ConfigError:
        return 1
    dirs = host_config['default-dirs']
...
def check_database(host_config):
    sys.path.append(host_config['default-dirs']['monetdb']['python'])
    try:
        verify_database()
...
def create_dirs(host_config):
    ...
    try:
        ...
        return 4, {}
    host_config['default-dirs'] = dirs
    return 0, host_config


def create_clusterdescription(path, host):
    if host == 'heastro':
        with open(path, 'w') as outfile:
            outfile.write("""\
ClusterName = heastro
...
def check_clusterdescription(host_config):
    if not os.path.exists(host_config['cdesc']):
        logging.warning("No cluster description file detected")
        if ask("Would you like to have a default one created") == 'y':
            if host_config['name'].startswith('heastro'):
                create_clusterdescription(host_config['cdesc'], 'heastro')
            logging.info("Created default cluster description file %s",
                         host_config['cdesc'])


def create_sipcfg(path, host, host_config):
    dirs = host_config['default-dirs']
    if host == 'heastro':
        with open(path, 'w') as outfile:
            outfile.write("""\
...
""" % (dirs['pipeline-runtime'], dirs['trap']['recipes'], dirs['sip']['recipes'],
       dirs['lofim']['base'], dirs['work'], host_config['cdesc'],
       dirs['tkp']['python'], dirs['lofim']['python'], dirs['monetdb']['python'],
       dirs['sip']['python'], dirs['tkp']['lib'], dirs['lofim']['lib']))
                          
def check_sipcfg(host_config):
    if not os.path.exists(host_config['sipcfg']):
        logging.warning("No SIP configuration file detected")
        if ask("Would you like to have a default one created") == 'y':
            if host_config['name'].startswith('heastro'):
                create_sipcfg(host_config['sipcfg'], 'heastro', host_config)
            logging.info("Created default SIP configuration file %s",
                         host_config['sipcfg'])


def create_job_layout(host_config):
    dirs = host_config['default-dirs']
    jobdir = os.path.join(dirs['jobs'], host_config['dsname'])
    if os.path.exists(jobdir):
        logging.warning("Directory %s already exists. Skipping.", jobdir)
        return -1
    try:
        os.makedirs(jobdir)
    except OSError:
...
def setup_control_dir(host_config):
    dirs = host_config['default-dirs']
    controldir = os.path.join(dirs['jobs'], host_config['dsname'], 'control')
...
# Remove old stuff; only do this automatically when testing
rm -r %s/*
rm -r %s/*
rm -r %s/*
rm %s
...
[prettyprint]
recipe = prettyprint
""" % (host_config['postgres']['host'],
       host_config['user']))
        

def create_default_parsets(host_config):
    dirs = host_config['default-dirs']
    parsetdir = os.path.join(dirs['jobs'], host_config['dsname'], 'parsets')
...
def find_subbands(host_config):
    if host_config['name'].startswith('heastro'):
        archivedir = os.path.join(host_config['default-dirs']['archive'], host_config['dsname'])
        datafiles = sorted(os.listdir(archivedir))
    # we could dump it with str(datafiles), but let's do it nicely
    with open(os.path.join(host_config['default-dirs']['jobs'],
                           host_config['dsname'], 'control', 'ms_to_process.py'),
              'w') as outfile:
...
def main():
    logging.info("Determining default setup")
    logging.info("")
    host_config = setup_host_config()    
    result = check_tkp_installation(host_config)
    if result:
        return result
    result = check_database(host_config)
    if result:
        return result
    show_config(host_config)
    logging.info("")
    result, host_config = create_dirs(host_config)
    if result:
        return result
    host_config['cdesc'] = os.path.join(host_config['default-dirs']['pipeline-runtime'], host_config['cdesc'])
    host_config['sipcfg'] = os.path.join(host_config['default-dirs']['pipeline-runtime'], host_config['sipcfg'])
    check_clusterdescription(host_config)
    check_sipcfg(host_config)
    if ask("Create a job directory structure for a new dataset") == 'y':
        host_config['dsname'] = ask("Dataset name", default=None, options=[])
        if create_job_layout(host_config) == 0:
            setup_control_dir(host_config)
            if ask("Create default parsets") == 'y':
                create_default_parsets(host_config)
            if ask("Try to find subbands based on dataset name") == 'y':
                find_subbands(host_config)
