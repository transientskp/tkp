...
begin
  ...
  return table (
    select 3600 * DEGREES(2 * ASIN(SQRT( (x1 - x2) * (x1 - x2)
          ...
          ,SQRT( (ira1 * COS(RADIANS(idecl1)) - ira2 * COS(RADIANS(idecl2))) * (ira1 * COS(RADIANS(idecl1)) - ira2 * COS(RADIANS(idecl2)))
                 / (ira_err1 * ira_err1 + ira_err2 * ira_err2)
               + (idecl1 - idecl2) * (idecl1 - idecl2)  
                 / (idecl_err1 * idecl_err1 + idecl_err2 * idecl_err2)
               ) AS assoc_r
          ,LOG10(EXP((( (ira1 * COS(RADIANS(idecl1)) - ira2 * COS(RADIANS(idecl2))) * (ira1 * COS(RADIANS(idecl1)) - ira2 * COS(RADIANS(idecl2))) 
                        / (ira_err1 * ira_err1 + ira_err2 * ira_err2)
                       + (idecl1 - idecl2) * (idecl1 - idecl2)  
                        / (idecl_err1 * idecl_err1 + idecl_err2 * idecl_err2)
                      )
                 ...
                 /
                 (2 * PI() * SQRT(ira_err1 * ira_err1 + ira_err2 * ira_err2) * SQRT(idecl_err1 * idecl_err1 + idecl_err2 * idecl_err2) * 4.02439375E-06)
                ) AS assoc_log_lr
  )
  ;
