...
BEGIN
  ...
  SELECT x2.xtrsrcid
   ...
   WHERE x1.xtrsrcid = ixtrsrcid
     ...
     AND x2.x * x1.x + x2.y * x1.y + x2.z * x1.z > COS(rad(itheta))
     ...
     AND x2.decl BETWEEN x1.decl - itheta
                     AND x1.decl + itheta
     AND POWER((COS(rad(x2.decl)) * COS(rad(x2.ra))
               - COS(rad(x1.decl)) * COS(rad(x1.ra))
               ), 2)
        + POWER((COS(rad(x2.decl)) * SIN(rad(x2.ra))
                - COS(rad(x1.decl)) * SIN(rad(x1.ra))
                ), 2)
        + POWER((SIN(rad(x2.decl))
                - SIN(rad(x1.decl))
                ), 2) = (SELECT MIN(POWER((COS(rad(x2.decl)) * COS(rad(x2.ra))
                                          - COS(rad(x1.decl)) * COS(rad(x1.ra))
                                          ), 2)
                                   + POWER((COS(rad(x2.decl)) * SIN(rad(x2.ra))
                                           - COS(rad(x1.decl)) * SIN(rad(x1.ra))
