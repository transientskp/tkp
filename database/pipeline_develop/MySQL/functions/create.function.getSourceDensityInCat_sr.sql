DROP FUNCTION IF EXISTS getSourceDensityInCat_sr;

DELIMITER //

/*
 * This function returns the source density in the neighborhood
 ...
 * the neighborhood determine the source density [sr^-1].
 */
CREATE FUNCTION getSourceDensityInCat_sr(icatname VARCHAR(50)
                                        ,itheta DOUBLE
                                        ,ixtrsrcid INT
                                        ) RETURNS DOUBLE
DETERMINISTIC
BEGIN
   ...
   */
  SET caparea = 2 * PI() * (1 - COS(RADIANS(itheta)));
