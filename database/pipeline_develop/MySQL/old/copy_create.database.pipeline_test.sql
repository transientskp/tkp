 ...
 *|     - LOAD all the sources from the catalog database              |
 ...
 *|       processing time)                                            |
 *|     - call the procedure InitObservation(zoneheight, theta) to    |
 *|       fill the spatial search tables, and others,                 |
 *|       default is "CALL BuildZones(1, 1);"                         |
 *| () During observation - Use the database                          |
 *|     - Before the extracted sources will be inserted, they will be |
 *|       inspected on associations with previous sources. After the  |
 *|       end of procedure AssociateSource() the sources will be      |
 *|       inserted.                                                   |
 *|     - AssociateSource() will associate them with previous inserted|
 *|       extracted sources (those in EXTRACTEDSOURCES) and with      |
 *|       catalogued sources (those in CATALOGUESOURCES). Inspection  |
 *|       will be done in time, frequency, and integration time (last |
 *|       not implemented yet).                                       |
 *|     - Transient detection                                         |
 ...
 *| (5) WHERE and WHEN do we want to construct the lightcurves?       |
 *|     or: do the lightcurves exist in the pipeline db (a)           |
 *|         or do they exist in the catalog db (b) ?                  |
 *|     if (a): we have to import all data again                      |
 *|        (b): data stays at its place                               |
 *|                                                                   |
 *+-------------------------------------------------------------------+
 */
...
INSERT INTO versions 
  ...
  ("0.0.1"
  ,(SELECT now())
  ,"/pipe/database/pipeline/MySQL/create.database.sql"
  );
...
/**
 * This table contains the files that are being processed
 * The only format for now is FITS. The HDF5 format will be implemented
 * later.
 * A file is characterised by the integration time (tau), the frequency
 * band (band) and the timestamp (seq_nr).
 * A group of files that belong together (not specified any further)
 * are in the same dataset (they have the same ds_id).
...
CREATE TABLE files (
  ...
  PRIMARY KEY (taustart_timestamp
              ,tau
              ,band
              ),
  UNIQUE INDEX (fileid),
...
/**
 * This table contains the catalogue sources that are associated.
 ...
 */
CREATE TABLE cataloguedsources (
...
CREATE TABLE extractedsources (
  ...
  INDEX (file_id),
  FOREIGN KEY (file_id) REFERENCES files (fileid)
) ENGINE=InnoDB;
...
CREATE TABLE associatedsources (
  ...
  FOREIGN KEY (assoc_catsrcid) REFERENCES cataloguedsources (catsrcid),
  INDEX (assoc_class_id),
  FOREIGN KEY (assoc_class_id) REFERENCES associationclass (assoc_classid)
) ENGINE=InnoDB;

/**
 * This table stores the information about the major catalogue sources
 ...
 *                  catalogue source 
 * assoc_catsrcid:  This is the id of another catalogue source that 
 *                  could be associated with the catalogue source
 *                  (corresponding to assoc_xtrsrcid)
 * src_id:          Reference to the source in the sourcelist table.
 *                  This is an averaged source according to the 
 *                  associations.
 * 
 * TODO: What catalogues do we keep here?
 * WENSS, VLSS, NVSS, 8C, ...?
 */
CREATE TABLE associatedcatsources (
  ...
  FOREIGN KEY (catsrc_id) REFERENCES cataloguedsources (catsrcid),
  FOREIGN KEY (assoc_catsrcid) REFERENCES cataloguedsources (catsrcid),
...
/*+-------------------------------------------------------------------+
 *| Because MySQL does not support subsecond timestamps, we have to   |
 *| create these fucntion ourselves.                                  |
 *| It is assumed that a timestamp is represented as a BIGINT with    |
 *| the format "YYYYMMDDHHmmSSmmm", so date, time, milliseconds.      |
...
/**
 * TODO: More functions to build, as we will need them
 */
...
BEGIN
  ...
  DECLARE iassoc_seq_xtrsrcid INT DEFAULT NULL;
  DECLARE nassoc_seq_xtrsrcid INT DEFAULT 0;
  DECLARE iassoc_seq_catsrcid INT DEFAULT NULL;
  DECLARE nassoc_seq_catsrcid INT DEFAULT 0;

  DECLARE iband INT;

  DECLARE assoc_seq_xtr BOOLEAN DEFAULT TRUE;
  DECLARE assoc_seq_cat BOOLEAN DEFAULT TRUE;

  DECLARE found BOOLEAN DEFAULT FALSE;
  DECLARE sigma INT DEFAULT 1;
  DECLARE iassoc_class_id INT DEFAULT 0;
  DECLARE cat_assoc_class_id INT DEFAULT 0;
  DECLARE xtr_assoc_class_id INT DEFAULT 0;
  ...
  SET itheta = GREATEST(ira_err, idecl_err);
   ...
   * are internal.
   */
  SET assoc_seq_cat = TRUE;
  SET assoc_seq_xtr = TRUE;

  IF (assoc_seq_cat = TRUE) THEN
    /**
    ...
    WHILE (found = FALSE AND sigma < 4) DO
      ...
      SELECT COUNT(*)
       ...
       WHERE zone BETWEEN FLOOR((idecl - itheta)/izoneheight)
         ...
         AND cat_id = icat_id
      ;
      IF nassoc_seq_catsrcid > 0 THEN
        SELECT catsrcid
          INTO iassoc_seq_catsrcid
          FROM cataloguesources
         WHERE zone BETWEEN FLOOR((idecl - itheta)/izoneheight)
                        AND FLOOR((idecl + itheta)/izoneheight)
           AND ra BETWEEN ira - ialpha
           ...
           AND DEGREES(ACOS(ix * x + iy * y + iz * z)) = 
              (SELECT MIN(DEGREES(ACOS(ix * x + iy * y + iz * z)))
                ...
                WHERE zone BETWEEN FLOOR((idecl - itheta)/izoneheight)
                  ...
                  AND decl BETWEEN idecl - itheta
                               AND idecl + itheta
                  /*TODO: Change to sin distance check*/
                  AND (ix * x + iy * y + iz * z) > COS(RADIANS(itheta))
                  AND cat_id = icat_id
              )
        ;
    ...
    IF nassoc_seq_catsrcid = 1 THEN
      SET cat_assoc_class_id = 1000 + 100 * (sigma - 1);
    ELSEIF nassoc_seq_catsrcid > 1 THEN
      SET cat_assoc_class_id = 2000 + 100 * (sigma - 1);
    END IF;
  ...
  IF (assoc_seq_xtr = TRUE) THEN
    ...
    WHILE (found = FALSE AND sigma < 4) DO
      ...
      ;
      IF nassoc_seq_xtrsrcid > 0 THEN
        SELECT assoc_seq_xtrsrcid
          INTO iassoc_seq_xtrsrcid
          FROM extractedsources
         WHERE zone BETWEEN FLOOR((idecl - itheta)/izoneheight) 
                        AND FLOOR((idecl + itheta)/izoneheight)
           AND ra BETWEEN ira - ialpha 
  ...
  IF nassoc_seq_xtrsrcid = 1 THEN
    SET xtr_assoc_class_id = 10 + sigma - 1;
  ELSEIF nassoc_seq_xtrsrcid > 1 THEN
    SET xtr_assoc_class_id = 20 + sigma - 1;
  END IF;

  SET iassoc_class_id = cat_assoc_class_id + xtr_assoc_class_id;
  ...
  /**
   * Only if iassoc_seq_xtrsrcid was NULL, we will set it to
   * the xtrsrcid value. We have to do an update because
   * xtrsrcid is defined as AUTO_INCREMENT
   */
  UPDATE extractedsources
     SET assoc_seq_xtrsrcid = xtrsrcid
   WHERE assoc_seq_xtrsrcid IS NULL
  ;
...
/*
 * This boolean function returns TRUE if two ellipses intersect
 * or overlap, and FALSE if they do not intersect and not overlap.
 * The input args are the central coordinates (ra,decl) and
 * the errors on these, which determine the semi-major and semi-
 * minor axes.
 */
