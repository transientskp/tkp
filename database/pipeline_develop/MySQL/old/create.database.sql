 ...
 *|                                                                   |
 *| This script creates the TKP "pipeline" MySQL database. (An        |
 *| analog database exists for MonetDB.)                              |
 ...
 *|     - The stored procedure AssociateSource() will associate       |
 *|       sources with previously inserted extracted sources in       |
 *|       EXTRACTEDSOURCES AND catalogue sources that are in the      |
 *|       CATALOGUESOURCES table.                                     |
 *|     - Transient detection                                         |
...
/**
 * Create the "pipeline" test database
 */
...
INSERT INTO versions 
  ...
  ("0.0.1"
  ,(SELECT now())
  ,"/pipe/database/create.database.sql"
  );
