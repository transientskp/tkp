DROP PROCEDURE IF EXISTS Assocs2CatSourcesInRegion;
 ...
 *  
 */
CREATE PROCEDURE Assocs2CatSourcesInRegion(IN iimage_id INT
...
BEGIN
  ...
  SET oregion_file = CONCAT(idirname, DATE_FORMAT(NOW(), '%Y%m%d-%H%i'), 'assoc', iimage_id, ".reg");
  ...
  SET oregion_sql = CONCAT("SELECT '# Region file format: DS9 version 4.0'"
                          ...
                          ," SELECT 'fk5'"
                          ," UNION"
                          ," SELECT CONCAT(\'circle(\', x1.ra, \',\', x1.decl, \', 0.025) #color="
                          ,icolor
                          ,", text={\', id,\'}\')"
                          ," INTO OUTFILE "
                          ,"'"
                          ...
                          ," LINES TERMINATED BY \'\\n\'"
                          ," FROM associatedsources, catalogedsources c1, extractedsources x1"
