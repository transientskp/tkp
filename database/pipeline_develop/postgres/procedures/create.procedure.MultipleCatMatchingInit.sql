--DROP PROCEDURE IF MultipleCatMatchingInit;
...
BEGIN
  ...
  DECLARE icat_id INT;
  DECLARE izoneheight, ira_min, ira_max, idecl_min, idecl_max double precision;

  DECLARE imultcatsrcid, imultcatsrcid1, imultcatsrcid2 INT;
  DECLARE imultcatassocid INT;
  DECLARE ira, idecl, ira_err, idecl_err double precision;
  DECLARE ira1, idecl1, ira1_err, idecl1_err double precision;
  DECLARE ira2, idecl2, ira2_err, idecl2_err double precision;
  DECLARE inew_ra, inew_decl, inew_ra_err, inew_decl_err double precision;
  DECLARE inew_zone INT;
  DECLARE isrc1_id, isrc2_id, inew_src_id, iother_src_id INT;
  DECLARE iweight double precision;
  ...
  WHILE (icat_id <= 6) DO
    INSERT INTO multiplecatalogsources
      ...
      /*cannot use non GROUP BY column 'decl' in query results without an aggregate function
