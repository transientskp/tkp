...
home = os.getenv('HOME') 
path = home + '/tkp-code/pipe/database/pipeline_develop/MonetDB/scripts/simrsm'

f = '/copy.into.extractedsources.' + str(ns) + '.csv'
infile = path + f

logtime = time.strftime("%Y%m%d-%H%M")
lf = '/logs/insert.assoc.' + host + '.' + str(ns) + '.sources.per.image.' + str(iter) + '.images.' + logtime + '.log'
logfile = path + lf
logf = open(logfile, 'w')
