 ...
 */
public class TestLoadExtractedSources {
	...
	/*protected static String insertExtractedSources = "INSERT INTO extractedsources  " +
						...
						//" ?             " +
                                                ...
                                                ",?             " +
                                                ")");*/
	protected static StringBuffer insertExtractedSources = new StringBuffer(
	 ...
	 */
		...
		int numOfInserts = Integer.parseInt(args[0]);	
		...
		try {
					...
					String append = " (" +
                                          ...
                                          src.getIntegrId() + ", " +
                                           src.getHtmId() + ", " +
                                             ...
                                             src.getStokesUErr() + ", " +
                                             src.getStokesVErr() + "),";
					insertExtractedSources.append(append);
					//insertXtrSrc(pstmtXtrSrc, src);
				}
				insertExtractedSources.replace((insertExtractedSources.length() - 1), insertExtractedSources.length(), ";");
				System.out.println("Query: " + insertExtractedSources.toString());
				insertBulk(st, insertExtractedSources);
				endDB = System.currentTimeMillis();
				endDBins = endDB;
				totalDB = totalDB + (endDB - startDB);
				totalDBins = totalDBins + (endDBins - startDBins);*/
			...
			totalDBins = (endDBins - startDBins);
	...
	private static void insertBulk(Statement st, StringBuffer query) throws SQLException {
		try {
			query.append(";");
			st.executeUpdate(query.toString());
                } catch (SQLException sqle) {
                        System.err.println("+-------------------------------------------------------");
                        System.err.println("| SQLException @ insertBulk()");
                        System.err.println("| SQLException ErrorCode: " + sqle.getErrorCode());
                        System.err.println("| SQLException SQLState: " + sqle.getSQLState());
                        System.err.println("| SQLException Message: " + sqle.getMessage());
                        System.err.println("| ------------------------------------------------------");
