...
class TestApi(unittest.TestCase):
    ...
    def test_last_assoc_timestamps(self):
        q = tkp.db.api._last_assoc_timestamps(self.session, self.dataset1)
        r = self.session.query(q).all()
        self.assertEqual(len(r), 2)  # we have two bands

    def test_last_assoc_per_band(self):
        q = tkp.db.api._last_assoc_per_band(self.session, self.dataset1)
        r = self.session.query(q).all()
        self.assertEqual(len(r), 2)  # we have two bands

    def test_last_ts_fmax(self):
        q = tkp.db.api._last_ts_fmax(self.session, self.dataset1)
        r = self.session.query(q).all()[0]
        self.assertEqual(r.max_flux, 0.01)

    def test_newsrc_trigger(self):
        q = tkp.db.api._newsrc_trigger(self.session, self.dataset1)
        self.session.query(q).all()

    def test_combined(self):
        q = tkp.db.api._combined(self.session, self.dataset1)
    ...
    def test_transient(self):
        r = tkp.db.api.transients(self.session, self.dataset1).all()
    ...
    def test_transient_region(self):
        ...
        Ra & Decl filtering
        """
        r = tkp.db.api.transients(self.session, self.dataset1, ra_range=(0, 2),
                                  decl_range=(0, 2)).all()
        self.assertEqual(len(r), 2)

        r = tkp.db.api.transients(self.session, self.dataset1, ra_range=(20, 22),
    ...
    def test_transient_cutoff(self):
        ...
        """
        r = tkp.db.api.transients(self.session, self.dataset1, v_int_min=0,
                                  eta_int_min=0).all()
        self.assertEqual(len(r), 2)

        q = tkp.db.api.transients(self.session, self.dataset1, v_int_min=1000,
                                  eta_int_min=0)
        r = q.all()
        self.assertEqual(len(r), 0)
    ...
    def test_transient_newsource(self):
        ...
        Ra & Decl filtering
        """
        r = tkp.db.api.transients(self.session, self.dataset1,
