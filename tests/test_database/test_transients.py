...
class TestTransientVariabilityIndicesNotNull(unittest.TestCase):
    """
    These tests check that the factors and the variability indices are not NULL.
    Relates to issue 4306.
    """
    ...
    def test_full_transient_search_varindices_notnull(self):
        bands = self.dataset.frequency_bands()
        self.assertEqual(len(bands), 1)
        #First run with zero limits:
        for i in range(3):
            if i == 0:
                eta_lim = 0.0
                V_lim = 0.0
            ...
            for t in transients:
                self.assertNotEqual(t['v_int'], None)
                self.assertNotEqual(t['eta_int'], None)
            ...
            for c in range(len(results)):
                for r in range(len(results[c])):
                  print results[c][r] 
                  self.assertNotEqual(results[c][r], None)
