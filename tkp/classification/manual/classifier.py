...
class Branch(object):
    ...
    def classify(self):
        tests = list(itertools.ifilter(lambda x: x.startswith('test'), dir(self)))
        for test in tests:
            try:
                for classification in (getattr(self, test))():
                    self.results.extend(classification(self.transient).classify())
...
class Classifier(object):
    ...
    def __init__(self, transient, base, *args, **kwargs):
        ...
        - base: The starting class from which the classification logic follows.
                The base class normally calls several other classes that classify
                various transient.
