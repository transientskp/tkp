        ...
        INSERT INTO temprunningcatalog
          ...
          SELECT t0.xtrsrc_id
            ...
            FROM (SELECT b0.xtrsrc_id as xtrsrc_id
                        ...
                        ,((datapoints * b0.avg_wra + x0.ra / (x0.ra_err * x0.ra_err)) / (datapoints + 1))
                         /
                         ((datapoints * b0.avg_weight_ra + 1 / (x0.ra_err * x0.ra_err)) / (datapoints + 1))
                         AS wm_ra
                        ,((datapoints * b0.avg_wdecl + x0.decl / (x0.decl_err * x0.decl_err)) / (datapoints + 1))
                         /
                         ((datapoints * b0.avg_weight_decl + 1 / (x0.decl_err * x0.decl_err)) / (datapoints + 1))
                         AS wm_decl
                        ,SQRT(1 / ((datapoints + 1) * ((datapoints * b0.avg_weight_ra 
                                                       + 1 / (x0.ra_err * x0.ra_err)) / (datapoints + 1))
                                  )
                             ) AS wm_ra_err
                        ,SQRT(1 / ((datapoints + 1) * ((datapoints * b0.avg_weight_decl 
                                                       + 1 / (x0.decl_err * x0.decl_err)) / (datapoints + 1))
                                  )
                             ) AS wm_decl_err
                        ,(datapoints * b0.avg_wra + x0.ra / (x0.ra_err * x0.ra_err)) 
                         / (datapoints + 1) AS avg_wra
                        ,(datapoints * b0.avg_wdecl + x0.decl / (x0.decl_err * x0.decl_err)) 
                         / (datapoints + 1) AS avg_wdecl
                        ,(datapoints * b0.avg_weight_ra + 1 / (x0.ra_err * x0.ra_err)) 
                         / (datapoints + 1) AS avg_weight_ra
                        ,(datapoints * b0.avg_weight_decl + 1 / (x0.decl_err * x0.decl_err)) 
                         / (datapoints + 1) AS avg_weight_decl
                        ,(datapoints * b0.avg_I_peak + x0.I_peak)
                         / (datapoints + 1) 
                         AS avg_I_peak
                        ,(datapoints * b0.avg_I_peak_sq + x0.I_peak * x0.I_peak)
                         / (datapoints + 1) 
                         AS avg_I_peak_sq
                        ,(datapoints * b0.avg_weight_peak + 1 / (x0.I_peak_err * x0.I_peak_err))
                         / (datapoints + 1)  
                         AS avg_weight_peak
                        ,(datapoints * b0.avg_weighted_I_peak + x0.I_peak / (x0.I_peak_err * x0.I_peak_err))
                         / (datapoints + 1) 
              ...
              ,(SELECT tb1.xtrsrc_id
                      ...
                      ,SQRT( (x1.ra - b1.wm_ra) * COS(rad(x1.decl)) 
                            *(x1.ra - b1.wm_ra) * COS(rad(x1.decl))
                            / (x1.ra_err * x1.ra_err + b1.wm_ra_err * b1.wm_ra_err)
                           + (x1.decl - b1.wm_decl) * (x1.decl - b1.wm_decl) 
                             / (x1.decl_err * x1.decl_err + b1.wm_decl_err * b1.wm_decl_err)
                           ) AS r1 
...
def __select_variability_indices(conn, dsid, V_lim, eta_lim):
    ...
    try: 
        ...
        SELECT xtrsrc_id
              ...
              ,wm_ra_err
              ,wm_decl_err
              ,sqrt(datapoints*(avg_I_peak_sq - avg_I_peak*avg_I_peak)/(datapoints-1))
               /avg_I_peak as V
              ,(datapoints/(datapoints-1)) *
               (avg_weighted_I_peak_sq - avg_weighted_I_peak*avg_weighted_I_peak/avg_weight_peak) 
               as eta
          FROM runningcatalog
         WHERE ds_id = %s
           AND datapoints > 1
           AND (sqrt(datapoints*(avg_I_peak_sq - avg_I_peak*avg_I_peak)/(datapoints-1))/avg_I_peak > %s
                OR (datapoints/(datapoints-1)) *(avg_weighted_I_peak_sq - avg_weighted_I_peak*avg_weighted_I_peak/avg_weight_peak) > %s
               )
        """
