...
class Island(object):
    ...
    # Each island needs to know its position in the image (ie, x,y pixel value
    # at one corner), the threshold above which it is detected
    # (analysis_threshold by default, but will increase if the island is the
    # result of deblending), and a data array.
    # The island should provide a means of deblending: splitting itself apart
    # and returning multiple sub-islands, if necessary.

    def __init__(self, data, rms, chunk, analysis_threshold, detection_map, beam, rms_orig=None, flux_orig=None,min_orig=None,max_orig=None):
        ...
        # The idea here is to retain the flux of the original, unblended island.
        # That flux is used as a criterion for deblending.
    ...
    def deblend(self,iter=1):
        ...
        for level in subthrrange:
            ...
            # Deblending is started at a level higher than the lowest pixel value in the island.
            # Deblending at the level of the lowest pixel value will likely not yield anything, because the
            # island was formed at a threshold just below that.
            ...
            if number > 1:
                ...
                for chunk in ndimage.find_objects(labels):
                    label += 1
                    newdata = numpy.where(labels == label, self.data.filled(fill_value=0.0), 0)
                    # NB: In class Island(object), rms * analysis_threshold is taken as the threshold
                    # for the bottom of the island. Everything below that level is masked.
                    # For subislands, this product should be equal to level and flat, i.e., horizontal.
                    # We can achieve this by setting rms=level*ones and analysis_threshold=1.
                    subislands.append(Island(newdata[chunk],
                        ...
                        self.detection_map[chunk], self.beam, self.rms_orig[chunk[0].start:chunk[0].stop,chunk[1].start:chunk[1].stop],self.flux_orig,self.min_orig,self.max_orig))

                # This line should filter out any subisland with insufficient flux,
                # in about the same way as Sextractor.
                ...
                if numbersignifsub>1:
                    ...
                    break # pull out of deblending loop, return current island.
...
class ParamSet(DictMixin):
    ...
    def __init__(self):
        self.values = {'peak': utilities.Uncertain(), 'flux': utilities.Uncertain(), 'xbar':\
            utilities.Uncertain(), 'ybar': utilities.Uncertain(), 'semimajor': utilities.Uncertain(),\
            'semiminor': utilities.Uncertain(), 'theta': utilities.Uncertain(), 'semimaj_deconv': \
            utilities.Uncertain(), 'semimin_deconv':  utilities.Uncertain(), 'theta_deconv': utilities.Uncertain()}
        # This parameter gives the number of components that could not be deconvolved, IERR from deconf.f.
    ...
    def __setitem__(self, item, value):
        ...
        elif item[:3] == 'err' and self.values.has_key(item[3:]):
    ...
    def calculate_errors(self, noise, beam, threshold):
        # Use __condon_formulae() if this object is based on a Gaussian fit,
        # __error_bars_from_moments() if it's based on moments.
        if self.gaussian:
            return self.__condon_formulae(noise, beam)
        elif self.moments:
            return self.__error_bars_from_moments(noise, beam, threshold)
        else:
            return False

    def __condon_formulae(self, noise, beam):
        # returns the errors on parameters from Gaussian fits according to the
        # Condon (PASP 109, 166 (1997)) formulae.  These formulae are not
        # perfect, but we'll use them for the time being.  (See Refregier and
        # Brown (astro-ph/9803279v1) for a more rigorous approach.) It also
        # returns the corrected peak.  The peak is corrected for the
        # overestimate due to the local noise gradient.
        ...
        rho_sq3=(smaj*smin/(theta_B*theta_b))*(1.+(theta_B/(2.*smaj))**2)**alpha_maj3*(1.+(theta_b/(2.*smin))**2)**alpha_min3*(peak/noise)**2
        ...
        # Here you get the errors parallel to the fitted semi-major and semi-minor axes as taken from the NVSS
        # paper (Condon et al. AJ, 115, 1693 (1998)), formula 25. Those variances are twice the theoreticals,
        # so the errors in position are sqrt(2) as large as one would get from formula 21 of the Condon (1997) paper.
        error_par_major=2.*smaj/denom1
        error_par_minor=2.*smin/denom2

        # When these errors are converted to RA and Dec, calibration uncertainties will have to be added,
        # like in formulae 27 of the NVSS paper.
        errorx=numpy.sqrt((error_par_major*numpy.sin(theta))**2+(error_par_minor*numpy.cos(theta))**2)
        errory=numpy.sqrt((error_par_major*numpy.cos(theta))**2+(error_par_minor*numpy.sin(theta))**2)

        # Note that we report errors in HWHM axes instead of FWHM axes so the errors are half the errors of formula 29 of the NVSS paper.
        errorsmaj=numpy.sqrt(2)*smaj/rho1
        errorsmin=numpy.sqrt(2)*smin/rho2

        if smaj>smin:
            errortheta=2.*(smaj*smin/(smaj**2-smin**2))/rho2
        else:
            errortheta=numpy.pi
        ...
        errorpeaksq=(frac_flux_cal_error*peak)**2+clean_bias_error**2+2.*peak**2/rho_sq3
    ...
    def __error_bars_from_moments(self, noise, beam, threshold):
        ...
        # Again, like above for the Condon formulae, we set the positional variances to twice the theoretical values.
        ...
        if smaj>smin:
            errortheta=2.*(smaj*smin/(smaj**2-smin**2))/rho
        else:
            errortheta=numpy.pi
        if errortheta>numpy.pi:
            errortheta=numpy.pi

        # The peak from "moments" is just the value of the maximum pixel times a correction, fudge_max_pix, for the fact that the centre of the Gaussian
        # is not at the centre of the pixel. This correction is performed in gaussian.py. The maximum pixel method introduces a peak dependent error corresponding to the last
    ...
    def deconvolve_from_clean_beam(self,beam):
        # If the fitted axes are smaller than the clean beam (=restoring beam) axes, the
        # axes and position angle can be deconvolved from it.
        ...
        if rmaj>0:
            ...
            # In the general case, where the restoring beam is elliptic, calculating the error bars of the deconvolved
            # position angle is more complicated than in the NVSS case, where a circular restoring beam was used.
            # In the NVSS case the error bars of the deconvolved angle are equal to the fitted angle.
            rmaj1, rmin1, rpa1, ierr1 = deconv(fmaj,fmin,fpa+fpaerror,cmaj,cmin,cpa)
            if ierr1<2:
                if rpa1>90:
                ...
                rpaerror1=numpy.nan
            rmaj2, rmin2, rpa2, ierr2 = deconv(fmaj,fmin,fpa-fpaerror,cmaj,cmin,cpa)
            if ierr2<2:
                if rpa2>90:
            ...
            if numpy.isnan(rpaerror1) or numpy.isnan(rpaerror2):
                self['theta_deconv'].error = numpy.nansum([rpaerror1,rpaerror2])
            else:
                self['theta_deconv'].error = numpy.mean([rpaerror1,rpaerror2])

            self['semimaj_deconv'].value = rmaj/2.
            rmaj3, rmin3, rpa3, ierr3 = deconv(fmaj+fmajerror,fmin,fpa,cmaj,cmin,cpa)
            # If rmaj>0, then rmaj3 should also be > 0, if I am not mistaken, see the formulas at
            # the end of ch.2 of Spreeuw's Ph.D. thesis.
            if fmaj-fmajerror>fmin:
                rmaj4, rmin4, rpa4, ierr4 = deconv(fmaj-fmajerror,fmin,fpa,cmaj,cmin,cpa)
                if rmaj4>0:
                    self['semimaj_deconv'].error = numpy.mean([numpy.abs(rmaj3-rmaj),numpy.abs(rmaj-rmaj4)])
                else:
                    self['semimaj_deconv'].error = numpy.abs(rmaj3-rmaj)
            else:
                rmin4, rmaj4, rpa4, ierr4 = deconv(fmin,fmaj-fmajerror,fpa,cmaj,cmin,cpa)
                if rmaj4>0:
                    self['semimaj_deconv'].error = numpy.mean([numpy.abs(rmaj3-rmaj),numpy.abs(rmaj-rmaj4)])
                else:
                    self['semimaj_deconv'].error = numpy.abs(rmaj3-rmaj)
            if rmin>0:
                ...
                if fmin+fminerror<fmaj:
                    rmaj5, rmin5, rpa5, ierr5 = deconv(fmaj,fmin+fminerror,fpa,cmaj,cmin,cpa)
                else:
                    rmin5, rmaj5, rpa5, ierr5 = deconv(fmin+fminerror,fmaj,fpa,cmaj,cmin,cpa)
                # If rmin>0, then rmin5 should also be > 0, if I am not mistaken, see the formulas at
                # the end of ch.2 of Spreeuw's Ph.D. thesis.
                rmaj6, rmin6, rpa6, ierr6 = deconv(fmaj,fmin-fminerror,fpa,cmaj,cmin,cpa)
                if rmin6>0:
                    self['semimin_deconv'].error = numpy.mean([numpy.abs(rmin6-rmin),numpy.abs(rmin5-rmin)])
                else:
...
def source_profile_and_errors(data, threshold, noise, beam, fixed={}):
    ...
    fitting and export these to a residual map.  And it will make a map of the
    decomposed sources.
    Input:
      - NumPy array of pixel values, can be a masked array, which is necessary
        for proper Gauss fitting, because the pixels below the threshold in
        the corners and along the edges should not be included in the fitting
        process;
      - Threshold used for selecting pixels for the source (ie, building an
        island) as a float;
      - Noise level in data as a float;
      - if fixed is a tuple (x,y) pass it on to gaussian.fitgaussian(): this
        will lock fit to only occur at that pixel coordinate.
...
Unable to estimate gaussian parameters. Proceeding with defaults %s""" %
    ...
    if (numpy.fabs(xmax-xmin)>2) and (numpy.fabs(ymax-ymin)>2):
        # Now we can do Gauss fitting if the island or subisland has a thickness
        # of more than 2 in both dimensions.
...
class Detection(object):
    ...
    def __init__(self, paramset, imagedata, chunk=None):
        ...
        try:
            ...
            logging.warn("Physical coordinates failed at %f,%f" % (self.x, self.y))
            raise

    def __getstate__(self):
        return {'imagedata': self.imagedata, 'chunk': (self.chunk[0].start,
        self.chunk[0].stop, self.chunk[1].start, self.chunk[1].stop), 'peak':
        self.peak, 'flux': self.flux, 'x': self.x, 'y': self.y, 'smaj':
        self.smaj, 'smin': self.smin, 'theta': self.theta, 'sig': self.sig}
    ...
    def __physical_coordinates(self):
        # Convert the pixel parameters for this object into something
        # physical.

        # First, the RA & dec.
        self.ra, self.dec = [Uncertain(x) for x in self.imagedata.wcs.p2s([self.x.value, self.y.value])]
        if numpy.isnan(self.dec.value) or abs(self.dec) > 90.:
        ...
        # The length of this vector is chosen such that it touches the tangent plane at center position.
        local_north_position=numpy.array([0.,0.,1./center_position[2]])
        # The cross product of the local north vector and the local east vector will always be aligned with the
        # center_position vector.

        # Next, determine the orientation of the y-axis wrt local north by incrementing y by a small amount and converting that
        # to celestial coordinates. That small increment is conveniently chosen to be an increment of 1 pixel.

        endy_ra,endy_dec=self.imagedata.wcs.p2s([self.x.value, self.y.value+1.])
        ...
        # This is the angle between the y-axis and local north, measured eastwards.
        # yoffset_angle=numpy.degrees(numpy.arcsin(numpy.sqrt(length_cross_sq/normalization)))
        # The formula above is commented out because the angle computed in this way will always be 0<=yoffset_angle<=90.
        # We'll use the dotproduct instead.
        yoffs_rad=numpy.arccos(numpy.dot(diff1,diff2)/numpy.sqrt(normalization))

        # The multiplication with -sign_cor makes sure that the angle is measured eastwards (increasing RA), not westwards.
        sign_cor=numpy.dot(cross_prod,center_position)/numpy.sqrt(length_cross_sq)
        yoffs_rad*= -sign_cor
        yoffset_angle=numpy.degrees(yoffs_rad)

        # Now that we have the BPA, we can also compute the position errors properly, by projecting the errors in pixel
        # coordinates (x and y) on local north and local east.
        errorx_proj=numpy.sqrt((self.x.error*numpy.cos(yoffs_rad))**2+(self.y.error*numpy.sin(yoffs_rad))**2)
        errory_proj=numpy.sqrt((self.x.error*numpy.sin(yoffs_rad))**2+(self.y.error*numpy.cos(yoffs_rad))**2)

        # Now we have to sort out which combination of errorx_proj and errory_proj gives the largest errors in RA and Dec.
        end_ra1, end_dec1 = self.imagedata.wcs.p2s([self.x.value+errorx_proj, self.y.value])
        ...
        # First, the Gaussian in gaussian.py must be such that theta is measured from the positive y-axis
        # in the direction of negative x.
        # Secondly, x and y are defined such that the direction positive y-->negative x-->negative y-->positive x
        # is the same direction (counterclockwise) as (local) north-->east-->south-->west.
        # If these two conditions are matched, the formula below is valid.
        # Of course, the formula is also valid if theta is measured from the positive y-axis towards positive x
        # and both of these directions are equal (clockwise).
        self.theta_celes=utilities.Uncertain((numpy.degrees(self.theta.value)+yoffset_angle) % 180,numpy.degrees(self.theta.error))
        self.theta_dc_celes=utilities.Uncertain((self.theta_dc.value+yoffset_angle) % 180,numpy.degrees(self.theta_dc.error))

        # Next, the axes.
        # Note that the signs of numpy.sin and numpy.cos in the 4 expressions below are arbitrary.
        end_smaj_x = self.x.value - numpy.sin(self.theta.value) * self.smaj.value
        end_smaj_y = self.y.value + numpy.cos(self.theta.value) * self.smaj.value

        end_smin_x = self.x.value + numpy.cos(self.theta.value) * self.smin.value
        end_smin_y = self.y.value + numpy.sin(self.theta.value) * self.smin.value

        end_smaj_ra, end_smaj_dec = self.imagedata.wcs.p2s([end_smaj_x,end_smaj_y])
        end_smin_ra, end_smin_dec = self.imagedata.wcs.p2s([end_smin_x,end_smin_y])

        smaj_asec = coordinates.angsep(self.ra.value, self.dec.value, end_smaj_ra, end_smaj_dec)
        ...
        smin_asec = coordinates.angsep(self.ra.value, self.dec.value, end_smin_ra, end_smin_dec)
    ...
    def serializeAll(self):
        ...
        return (int(numpy.floor(self.dec.value/zh))
               ...
               ,self.ra.error
               ,self.dec.error
               ,numpy.cos(numpy.radians(self.dec.value)) * numpy.cos(numpy.radians(self.ra.value))
               ,numpy.cos(numpy.radians(self.dec.value)) * numpy.sin(numpy.radians(self.ra.value))
    ...
    def serializeAllFloats(self):
        ...
        return (int(numpy.floor(self.dec.value/zh))
               ...
               ,numpy.float(numpy.cos(numpy.radians(self.dec.value)) * numpy.cos(numpy.radians(self.ra.value)))
               ,numpy.float(numpy.cos(numpy.radians(self.dec.value)) * numpy.sin(numpy.radians(self.ra.value)))
    ...
    def htmid(self, level=5):
        return htmid.coord2HTM(htmid.dec2dec(self.dec), htmid.RA2RA(self.ra), level)

    def dbstore(self, conn):
        # Legacy code; not used in current scheme.
