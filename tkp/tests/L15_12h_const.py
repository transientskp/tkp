...
class L15_12hConstObs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        fitsfile = accessors.FitsFile(os.path.join(DATAPATH, 'L15_12h_const/observed-all.fits'),beam=(54./3600,54./3600,0.))
        self.myim = image.ImageData(fitsfile.data, fitsfile.beam, fitsfile.wcs)
        self.results = self.myim.sextract(det=10)
...
class L15_12hConstCor(unittest.TestCase):
    ...
    def setUp(self):
        # Beam here is a random beam, in this case the WENSS beam without the declination dependence.
        fitsfile = accessors.FitsFile(os.path.join(DATAPATH, 'L15_12h_const/corrected-all.fits'),beam=(54./3600,54./3600,0.))
        self.myim = image.ImageData(fitsfile.data, fitsfile.beam, fitsfile.wcs)
        self.results  = self.myim.sextract(det=10)
...
class L15_12hConstMod(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # all.
        # Beam here is a random beam, in this case the WENSS beam without the declination dependence.
        fitsfile = accessors.FitsFile(os.path.join(DATAPATH, 'L15_12h_const/model-all.fits'),beam=(54./3600,54./3600,0.))
        self.myim = image.ImageData(fitsfile.data, fitsfile.beam, fitsfile.wcs)
        self.results  = self.myim.sextract(det=5)
...
class FitToPointTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.my_im = image.ImageData(fitsfile.data, fitsfile.beam, fitsfile.wcs)
