...
def calcsigma(data, errors=None, mean=None, axis=None, errors_as_weight=False):
    ...
    if w is not None:
        ...
        sigma = numpy.sqrt(((data*data).sum(axis) - N*mean*mean)/(N-1))
    return mean, sigma


def sigmaclip(data, errors=None, niter=0, siglow=3., sighigh=3., use_median=False):
    """
    ...
    indices = numpy.ones(len(data.ravel()), dtype=numpy.bool).reshape(data.shape)
