...
class Uncertain(object):
    ...
    standard deviation...).  Numeric operators are overloaded to work with
    other Uncertain or numeric objects.  The uncertainty (error) must be
    small. Otherwise the linearization employed here becomes wrong."""
    ...
    def __pow__(self, other):
        if isinstance(other, Uncertain):
            v = self.value**other.value
            e = (
                (self.error * other.value * self.value**(other.value - 1.0))**2 +
                (other.error * numpy.log(self.value) * self.value**other.value)**2
            )**.5
            return Uncertain(v, e)
        else:
            return Uncertain(self.value**other,
